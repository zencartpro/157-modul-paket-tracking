...
// add columns to order table
    ...
    //check if track_day column exists - if not add it
    $sql ="SHOW COLUMNS FROM ".TABLE_ORDERS_STATUS_HISTORY." LIKE '%track_day%'";
    $result = $db->Execute($sql);
    if(!$result->RecordCount())
    {
        $sql = "ALTER TABLE ".TABLE_ORDERS_STATUS_HISTORY." ADD track_day int(2)";
        $db->Execute($sql);
    }
    
    //check if track_month column exists - if not add it
    $sql ="SHOW COLUMNS FROM ".TABLE_ORDERS_STATUS_HISTORY." LIKE '%track_month%'";
    $result = $db->Execute($sql);
    if(!$result->RecordCount())
    {
        $sql = "ALTER TABLE ".TABLE_ORDERS_STATUS_HISTORY." ADD track_month int(2)";
        $db->Execute($sql);
    }
    
    //check if track_year column exists - if not add it
    $sql ="SHOW COLUMNS FROM ".TABLE_ORDERS_STATUS_HISTORY." LIKE '%track_year%'";
    $result = $db->Execute($sql);
    if(!$result->RecordCount())
    {
        $sql = "ALTER TABLE ".TABLE_ORDERS_STATUS_HISTORY." ADD track_year int(4)";
        $db->Execute($sql);
    }
